...
export const updateRuleInModule = async (
    ...
    try {
        ...
        const updatedProject = await ProjectModel.findOneAndUpdate(
            { _id: projectId, "modules._id": moduleId, "modules.rules._id": ruleId },
            { $set: { "modules.$[module].rules.$[rule]": updatedRuleData } },
            { arrayFilters: [{ "module._id": moduleId }, { "rule._id": ruleId }], new: true }
        );
        ...
        const updatedModule = updatedProject.modules.find((mod: any) => mod._id == moduleId);
